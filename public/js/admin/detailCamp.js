$(function () {
    ...
    // Jquery for chart
    let customer = $("#idDetailCamp").attr("customer");
    let data = JSON.parse(customer);
    detailCampaign(data, data.ob_campaign);
...
let detailCampaign = (data, elementCampaign) => {
    ...
    for (let j = 0; j < arr_shortUrl.fb.length; j++) {
        $(".block1").append('<label for="" style ="color: white;"> ' + (j + 1) + '. ' + arr_shortUrl.fb[j].group + ':</label>');
        $(".block1").append('<br>');
